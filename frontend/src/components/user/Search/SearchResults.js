        ...
        <Fragment>
            { thesis && thesis.map((theses) => (
                ...
                <Row>
                    ...
                    { isLoggedInGuest &&  (subTypeGuest && (subTypeGuest.status === 'Denied' || subTypeGuest.status === 'Expired') )  ? <h5> <Link to='#' data-target='#subscriptionModal' data-toggle="modal"> {theses.title} </Link> </h5> : null}
