...
export const fetchGuestCount = (user) => async (dispatch) => {
    try {

        dispatch({ type: GUEST_COUNT_REQUEST })


        const {data} = await axios.get( process.env.REACT_APP_URL + `/api/guest/count/${user}`)

        dispatch({
            type: GUEST_COUNT_SUCCESS,
            payload: data
        })
